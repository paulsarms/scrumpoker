...
export const PLAYER_SPEED = 0.012; // Normalized units per tick
export const BULLET_SPEED = 0.025; // Faster than players
export const PLAYER_RADIUS = 0.04; // For collision
...
export function processTick(game) {
  ...
  for (const player of alivePlayers) {
    ...
    if (input.up) {
      player.x += Math.cos(player.rotation) * PLAYER_SPEED;
      player.y += Math.sin(player.rotation) * PLAYER_SPEED;
    }
    if (input.down) {
      player.x -= Math.cos(player.rotation) * PLAYER_SPEED * 0.5; // Slower backwards
      player.y -= Math.sin(player.rotation) * PLAYER_SPEED * 0.5;
    }

    // Clamp to arena
    player.x = clamp(player.x, ARENA_MIN, ARENA_MAX);
    player.y = clamp(player.y, ARENA_MIN, ARENA_MAX);

    // 2. Handle shooting
    if (input.shooting && now - player.lastShot >= SHOOT_COOLDOWN) {
